...
class NonStaticIntentCNN(BaseIntentCNN):
...
class StaticIntentCNN(BaseIntentCNN):
    def __init__(self,config,seq_length,num_classes):
        self.input_x = tf.placeholder(tf.int32, [None, seq_length], name='input_channel1_x')
        pooled_outputs = []
        for i, filter_size in enumerate(config.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                print("conv-maxpool-%s" % filter_size)
...
class MultiChnIntentCNN(object):
    def __init__(self, config, embedding_mode, seq_length, num_classes, vocab_size):
        self.config = config
        if embedding_mode == 0:#noe-static
            self.input_channel1_x = tf.placeholder(tf.int32, [None, seq_length], name='input_channel1_x')
            with tf.device('/cpu:0'):
                embedding = tf.get_variable('embedding', [vocab_size, self.config.embedding_dim])
                self.intput_x = tf.nn.embedding_lookup(embedding, self.input_channel1_x)
        elif embedding_mode == 1:#static
            self.intput_x = self.input_channel2_x
        elif embedding_mode == 2:#two
            self.input_channel1_x = tf.placeholder(tf.int32, [None, seq_length], name='input_channel1_x')
            with tf.device('/cpu:0'):
                embedding = tf.get_variable('embedding', [vocab_size, self.config.embedding_dim])
                embedding_inputs = tf.nn.embedding_lookup(embedding, self.input_channel1_x)
            self.intput_x = tf.stack([embedding_inputs, self.input_channel2_x], axis=3)
        ...
        for i, filter_size in enumerate(self.config.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                print("conv-maxpool-%s" % filter_size)
                if embedding_mode != 2:
                    conv = tf.layers.conv1d(self.intput_x, self.config.num_filters, filter_size)
                else:
                    conv = tf.layers.conv2d(self.intput_x, self.config.num_filters, [filter_size,self.config.embedding_dim])
                pooled = tf.reduce_max(conv, reduction_indices=[1])
                pooled_outputs.append(pooled)
